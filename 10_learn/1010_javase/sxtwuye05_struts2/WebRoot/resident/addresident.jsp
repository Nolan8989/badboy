  ...
  <head>
	...
	<script>
			...
			$(function(){
				...
				$.ajax({
					type:'post',
					url:'${pageContext.request.contextPath }/selectRoom!findAllBuild',
					dataType:'json',
					success:function(rs){
						var b = $("#buildid");
						b.children().remove("option[value!=0]");
						//alert(rs);
...
//						alert(bid);
						...
						$.ajax({
							type:'post',
							url:"${pageContext.request.contextPath }/selectRoom!findUnitByBuild",
							data:'bid='+bid,
							dataType:'json',
							success:function(rs){
							//alert(rs);
...
//						alert(bid);
						...
						$.ajax({
							type:'post',
							url:"${pageContext.request.contextPath }/selectRoom!findRoomByUnitId",
							data:'uid='+uid,
							dataType:'json',
							success:function(rs){
							//alert(rs);
...
<form action="${pageContext.request.contextPath }/residentServlet.action?cmd=addRs" method="post" enctype="multipart/form-data" name="rsdform" onsubmit="return checkRsdForm(this)">
<table width="90%" border="1" style="border-collapse:collapse"   bordercolor="#00FF99">
  ...
  <tr>
    ...
    <td>
<input id="d4311" class="Wdate" name="starttime" type="text" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'d4312\')||\'2020-10-01\'}'})"/>
<span id="startMsg"></span>
<input id="d4312" class="Wdate" name="endtime" type="text" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'d4311\')}',maxDate:'2020-10-01'})"/>
<span id="endMsg"></span>
</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><div align="right">
    	<input type="hidden" name="beanName" value="com.bjsxt.po.Resident">
